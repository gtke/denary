...
*/
    	...
    	if (separated.length==1 || (currenttimelong/60000-lastupdatetimelong/60000)>1){
			new Thread() {
				public void run() {
            		...
            		try {
            			Thread.sleep(2000);
    					if (bankIDS == "CBR"){NanoConverter.mContext.runLongProcessCBR();} else
	    			   	if (bankIDS == "NBU"){NanoConverter.mContext.runLongProcessNBU();} else
	    			   	if (bankIDS == "NBRB"){NanoConverter.mContext.runLongProcessNBRB();} else
	    			   	if (bankIDS == "BNM"){NanoConverter.mContext.runLongProcessBNM();} else
	    			   	if (bankIDS == "AZ"){NanoConverter.mContext.runLongProcessAZ();} else
	    			   	if (bankIDS == "ECB"){NanoConverter.mContext.runLongProcessECB();} else
	    			   	if (bankIDS == "FOREX") {NanoConverter.mContext.runLongProcessFOREX();}
    					Thread.sleep(2000);
		...
		Intent nanointent = new Intent(context, NanoConverter.class);
